...
from .purchase import PurchasePolicyBase, PurchaseLog
...
class ModelConfig:
    ...
    population_regulation: float = 1.0  # вероятность "выбытия" (регулирование количество выбытия (продажа, смерть, брак)) от 0 до 1

    # стельность: 1 гаусс + clamp (без циклов)
    ...
    def sample_gestation_days(self, rng: random.Random) -> int:
        ...
        if x < self.gestation_lo: return self.gestation_lo
        if x > self.gestation_hi: return self.gestation_hi
        return x
...
class Simulation:
    def __init__(
        ...
        random_seed: int = 42,
        record_monthly: bool = False,   # isMonth режим
        population_regulation: float = 1.0,  # вероятность "выбытия" (регулирование количество выбытия (продажа, смерть, брак)) от 0 до 1
    ):
    ...
    def step_day(self) -> None:
        ...
        for cow in self.herd:
            if cull_cow_combined(cow, self.today, self.file_path, rng=self.rng, population_regulation=self.cfg.population_regulation):
                cow.status = "culled"
    ...
    def _buy_pregnant_heifers(self, count: int, mode: str) -> None:
        ...
        for i in range(count):
            days_to_calving = self.rng.randint(self.cfg.purchased_days_to_calving_lo, self.cfg.purchased_days_to_calving_hi)
    ...
    def _record_metrics(self) -> None:
        ...
        dim_sum = 0
        culled_value = self._culled_since_last_record if self.record_monthly else self._culled_today
        ...
        self.history.append(DailyMetrics(
            day=self.today,
